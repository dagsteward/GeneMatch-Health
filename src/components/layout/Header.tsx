...
export function Header() {
  ...
  return (
    ...
    <header className="fixed top-0 z-50 w-full border-b border-border/60 bg-background/80 shadow-sm backdrop-blur-md">
      <nav className="mx-auto flex max-w-[1280px] items-center justify-between gap-x-6 px-4 py-4 sm:px-6 lg:px-10">
        ...
        <div className="hidden min-w-0 items-center gap-5 lg:flex xl:gap-7">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="whitespace-nowrap text-sm font-semibold text-muted-foreground transition-colors hover:text-secondary"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="hidden shrink-0 items-center gap-3 lg:flex">
          ...
          <Link
            href="/partnerships"
            className="shrink-0 whitespace-nowrap rounded-full bg-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground transition-transform active:scale-95"
          >
            Partner With Us
          </Link>
        </div>
