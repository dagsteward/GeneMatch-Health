...
export default async function ResearchPage() {
  const content = await getContentMap(defaultsFor("research."));
  const clinicalCommitments = JSON.parse(content["research.clinicalCommitments"]) as {
    title: string;
    description: string;
  }[];
  const universities = JSON.parse(content["research.universities.list"]) as string[];
  const roadmap = JSON.parse(content["research.roadmap"]) as {
    phase: string;
    status: string;
    description: string;
  }[];
        ...
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2">
          <Reveal>
            ...
            <p className="max-w-3xl text-lg text-muted-foreground">
              {content["research.hero.intro"]}
            </p>
          </Reveal>
          <Reveal delay={0.1}>
            <div className="relative aspect-[4/3] overflow-hidden rounded-3xl shadow-xl">
              <Image
                src={content["research.hero.image"]}
      ...
      <Section className="bg-card">
        <Reveal>
          ...
          <p className="mb-12 max-w-2xl text-muted-foreground">
            {content["research.universities.intro"]}
          </p>
        </Reveal>
