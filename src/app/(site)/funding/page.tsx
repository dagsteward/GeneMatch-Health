...
export default async function FundingPage() {
  const content = await getContentMap(defaultsFor("funding."));
  const fundingAvenues = JSON.parse(content["funding.avenues"]) as {
    title: string;
    description: string;
  }[];
  const revenueModel = JSON.parse(content["funding.revenueModel"]) as {
    stream: string;
    description: string;
  }[];
    ...
    <>
      <Section className="bg-card" withHelix>
        <Reveal>
          ...
          <p className="max-w-3xl text-lg text-muted-foreground">{content["funding.hero.intro"]}</p>
        </Reveal>
      </Section>
